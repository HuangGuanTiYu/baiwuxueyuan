...
- (UIImage *)trackBackgroundImage
{
    if(_trackBackgroundImage==nil)
    {
        ...
        else
        {
            ...
            image = [image resizableImageWithCapInsets:UIEdgeInsetsMake(0.0, 2, 0.0, 2)];
            _trackBackgroundImage = image;
        }
    }
...
-(BOOL)continueTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event
{
    ...
    if(_lowerHandle.highlighted)
    {
        ...
        if(!_upperHandle.highlighted || newValue<_lowerValue)
        {
            ...
            NSString *start = [self getMMSSFromSS:[NSString stringWithFormat:@"%i",(int)(newValue + 0.5)]];
            
            if ([self.nMRangeSliderDelete respondsToSelector:@selector(nMRangeSliderStartChange:)]) {
                [self.nMRangeSliderDelete nMRangeSliderStartChange:start];
            }
    ...
    if(_upperHandle.highlighted )
    {
        ...
        if(!_lowerHandle.highlighted || newValue>_upperValue)
        {
            ...
            if ([self.nMRangeSliderDelete respondsToSelector:@selector(nMRangeSliderEndChange:)]) {
                [self.nMRangeSliderDelete nMRangeSliderEndChange:start];
            }
