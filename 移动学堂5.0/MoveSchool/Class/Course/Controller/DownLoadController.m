             ...
             if ([responseDic[@"data"][@"service"] intValue] == 2) {
                 ...
                 }else if (self.format == 2)
                 {
                     for (int i = 0 ; i < selectedArray.count; i ++) {
                         ...
                         NSString *urlPraise = [NSString stringWithFormat:@"%@%@?zttvid=%@&video_type=1",header,VideoInfo,model.ccid];
                         ...
                         [[AFNetWW sharedAFNetWorking] AFWithPostORGet:@"get" withURLStr:urlPraise WithParameters:nil success:^(id responseDic) {
                             ...
                             if (code == 10000)
                             {
                                 NSString *url = [NSString stringWithFormat:@"%@",responseDic[@"data"][@"url"]];
                                 [self downLoadMyVideo:url model : model];
...
- (void) downLoadMyVideo : (NSString *) url model : (ChapterModel *) model
{
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    ChapterModel *model = self.titles[indexPath.row];
    model.buttonIsSelected = !model.buttonIsSelected;
