...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        [self.liveButton setTitle:@"去直播" forState:UIControlStateNormal];
        [self.liveButton setTitleColor:MainColor forState:UIControlStateNormal];
        self.liveButton.titleLabel.font = [UIFont systemFontOfSize:ys_f24];
        self.liveButton.layer.borderColor = MainColor.CGColor;
        self.liveButton.layer.borderWidth = 0.5;
        self.liveButton.layer.cornerRadius = 1.5;
        self.liveButton.layer.masksToBounds = YES;
        [self.liveButton addTarget:self action:@selector(liveButtonClick) forControlEvents:UIControlEventTouchUpInside];
...
- (void)layoutSubviews
{
    ...
    self.liveButton.frame = CGRectMake(0, 0, 45, 22);
    self.liveButton.centerY = self.contentView.height * 0.5;
    self.liveButton.x = self.contentView.width - mainSpacing15 - 45;
}
...
- (void)setModel:(HomePageModel *)model
{
    ...
    if (self.isFromStudy && [self.teacherid isEqualToString:[UserInfoTool getUserInfo].zttid] && [model.living isEqualToString:@"1"]) {
...
- (void)setIsFromStudyDetail:(BOOL)isFromStudyDetail
{
    _isFromStudyDetail = isFromStudyDetail;
    
    self.subtitleLabel.hidden = isFromStudyDetail;
}

- (void)setIsFromStudy:(BOOL)isFromStudy
{
    _isFromStudy = isFromStudy;
...
- (void) liveButtonClick
{
    ...
    if ([self.delegate respondsToSelector:@selector(liveButtonClick:)]) {
        [self.delegate liveButtonClick:self];
    }
}
