...
- (void) setUpData
{
    NSString *likeUrl = [NSString stringWithFormat:@"%@%@?index=%@&count=10&token=%@&labelClassid=%@&labelids=0",NetHeader,GetCoursesList,[NSString stringWithFormat:@"%d",self.index],[UserInfoTool getUserInfo].token,[NSString stringWithFormat:@"%@",self.ID]];
    
    if (self.isFromSearch) {
        likeUrl = [NSString stringWithFormat:@"%@%@?key=%@&businesscode=%@&index=%@&count=10&token=%@",NetHeader,SearchKey,self.key,courseType,[NSString stringWithFormat:@"%d",self.index],[UserInfoTool getUserInfo].token];
