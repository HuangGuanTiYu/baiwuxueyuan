...
@property (nonatomic, strong) TopImageButton *nextButton;
...
//    CGRect frame = CGRectMake(0, 0, SCREEN_HEIGHT, SCREEN_WIDTH);
    ...
    //播放按钮
    UIButton *startButton = [[UIButton alloc] initWithFrame:CGRectMake(15, 0, 30, 30)];
    ...
    // 视频总时间
    UILabel *durationLabel = [[UILabel alloc] initWithFrame:CGRectMake(footerView.width - 60 - 15 - 30, 0, 60, 20)];
    ...
    UIButton *fullButton = [[UIButton alloc] initWithFrame:CGRectMake(footerView.width - 30 - mainSpacing, 0, 30, 30)];
    ...
    fullButton.centerY = footerView.height * 0.5;
    [fullButton setImage:[UIImage iconWithInfo:TBCityIconInfoMake(@"\U0000e649", 18, [UIColor whiteColor])] forState:UIControlStateNormal];
    [fullButton setImage:[UIImage iconWithInfo:TBCityIconInfoMake(@"\U0000e64a", 18, [UIColor whiteColor])] forState:UIControlStateSelected];
...
- (void) backButtonClick
{
    ...
    if ([self.delegate respondsToSelector:@selector(fullButtonClick:)]) {
        [self.delegate fullButtonClick:YES];
    }
}
...
- (void) fullClick : (UIButton *) button
{
    ...
    if ([self.delegate respondsToSelector:@selector(fullButtonClick:)]) {
        [self.delegate fullButtonClick:button.selected];
    }
...
- (void) changeFrame : (BOOL) isFull {
    ...
    [[UIApplication sharedApplication] setStatusBarHidden:isFull withAnimation:NO];

    self.isSmall = !isFull;
    
    self.videoBackgroundView.frame = self.view.bounds;
    self.overlayView.frame = self.view.bounds;
    self.player.view.frame = self.view.bounds;
    self.footerView.frame = CGRectMake(0, self.overlayView.height - 50, self.overlayView.width, 50);
    self.durationLabel.x = self.footerView.width - 60 - 15 - 30;
    self.fullButton.x = self.footerView.width - 30 - mainSpacing;
    self.durationSlider.width = self.durationLabel.x - mainSpacing * 2 - CGRectGetMaxX(self.currentPlaybackTimeLabel.frame);
    self.headerView.width = self.overlayView.width;
    self.qualityButton.x = self.headerView.width - 60 - 22;
    self.backButton.x = mainSpacing;
    self.qualityView.x = self.qualityButton.x;
    self.nextMaskView.frame = self.overlayView.bounds;
    self.nextButton.centerX = self.overlayView.width * 0.5;
    self.nextButton.centerY = self.overlayView.height * 0.5;
}
