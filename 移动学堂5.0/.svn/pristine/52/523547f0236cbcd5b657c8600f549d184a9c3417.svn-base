...
- (void)drawRect:(CGRect)rect {
	...
	for (int i = 0; i < _numOfV; i++) {
        ...
        if (__IPHONE_OS_VERSION_MIN_REQUIRED >= 70000) {
            ...
            rect=CGRectMake(legendCenter.x - width / 2.0,
                            legendCenter.y - height / 2.0,
                            width,
                            height);
        ...
        [btn addTarget:self action:@selector(pushtotype:) forControlEvents:UIControlEventTouchUpInside];
    ...
    if (_numOfV > 0) {
        for (int serie = 0; serie < [_dataSeries count]; serie++) {
            ...
            for (int i = 0; i < _numOfV; ++i) {
                ...
                if (i == 0) {
                    CGContextMoveToPoint(context, _centerPoint.x, _centerPoint.y - (90 - _minValue) / (_maxValue - _minValue) * _r);
                }
                else {
            ...
            CGContextAddLineToPoint(context, _centerPoint.x, _centerPoint.y - (90 - _minValue) / (_maxValue - _minValue) * _r);
            ...
            if (_drawPoints) {
                for (int i = 0; i < _numOfV; i++) {
                    CGFloat value = [_dataSeries[serie][i] floatValue];
                    CGFloat xVal = _centerPoint.x - (90 - _minValue) / (_maxValue - _minValue) * _r * sin(i * radPerV);
                    CGFloat yVal = _centerPoint.y - (90 - _minValue) / (_maxValue - _minValue) * _r * cos(i * radPerV);
...
-(void)pushtotype:(UIButton *)btn{
//    self.block(btn.tag);
    [self.delegate courseclick:btn.tag];
